...
var en$1 = {
	"HueSmartLight.entry.name": "Hue Smart Light",
	"HueSmartLight.entry.description": "Description of this extension"
};
var ja$1 = {
	"HueSmartLight.entry.name": "Hue Smart Light",
	"HueSmartLight.entry.description": "拡張機能の説明"
};
var translations$1 = {
	en: en$1,
	ja: ja$1,
	"ja-Hira": {
	"HueSmartLight.entry.name": "Hue Smart Light",
	"HueSmartLight.entry.description": "かくちょうきのうのせつめい"
}
};
...
var entry = {
  get name() {
    return formatMessage$1({
      id: 'HueSmartLight.entry.name',
      default: 'Hue Smart Light',
      description: 'name of the extension'
    });
  },

  extensionId: 'HueSmartLight',
  extensionURL: 'https://Qumacky.github.io/xcx-smart-light/dist/HueSmartLight.mjs',
  ...
  get description() {
    return formatMessage$1({
      ...
      id: 'HueSmartLight.entry.description'
    });
  },
  ...
  helpLink: 'https://Qumacky.github.io/xcx-smart-light/',
...
var en = {
	"HueSmartLight.name": "Hue Smart Light",
	"HueSmartLight.doIt": "do it [SCRIPT]"
};
var ja = {
	"HueSmartLight.name": "Hue Smart Light",
	"HueSmartLight.doIt": "[SCRIPT] を実行する"
};
var translations = {
	en: en,
	ja: ja,
	"ja-Hira": {
	"HueSmartLight.name": "Hue Smart Light",
	"HueSmartLight.doIt": "[SCRIPT] をじっこうする"
}
};
...
var EXTENSION_ID = 'HueSmartLight';
/**
...
var extensionURL = 'https://Qumacky.github.io/xcx-smart-light/dist/HueSmartLight.mjs';
/**
...
var ExtensionBlocks = /*#__PURE__*/function () {
  /**
   * Construct a set of blocks for Hue Smart Light.
  ...
  createClass(ExtensionBlocks, [{
    ...
    value: function getInfo() {
      ...
      return {
        ...
        blocks: [{
          ...
          text: formatMessage({
            id: 'HueSmartLight.setDevicePower',
            default: 'IP Address [HOST] username [NAME] color [COLOR]',
            description: ''
          }),
          func: 'setDevicePower',
          arguments: {
    ...
    value: function setDevicePower(args) {
      var host = cast.toString(args.HOST);
      log.log(host);
      var username = cast.toString(args.NAME);
      log.log(username);
      ...
      if (min !== max) {
        if (r === max) {
          h = Math.floor(60 * (g - b) / (max - min) % 360 / 360 * range_h); // 0 〜 100

          h = h < 0 ? h + range_h : h;
        } else if (g === max) {
          h = Math.floor((60 * (b - r) / (max - min) + 120) % 360 / 360 * range_h);
        } else if (b === max) {
          h = Math.floor((60 * (r - g) / (max - min) + 240) % 360 / 360 * range_h);
        }

        s = Math.floor((max - min) / max * range_s);
      }

      var v = Math.floor(max / 255 * range_v);
      var hsb = {
        h: h,
        s: s,
        b: v
      ...
      log.log(hsb); // http サーバとの通信
      // const body = {host:'http://192.168.12.7', username:'5SxbisbyGZ9ve5dXxEs-afXPRbDiuvHDmbOacGdp',
      //               data:{on:true, sat:227, bri:127, hue:12616}};
      ...
      var url = new URL('http://localhost:5001');
      url.pathname = 'state';
      log.log(body);
